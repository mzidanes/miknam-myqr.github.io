...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if ($id == "settings" && explode("-",$router)[0] == "new") {
    ...
    fwrite($f, "\n'$'data['".$router."'] = array ('1'=>'".$router."!','".$router."@|@','".$router."#|#','".$router."%','".$router."^','".$router."&Rp','".$router."*10','".$router."(1','".$router.")','".$router."=','".$router."@!@disable','".$router."<','".$router."?','".$router."|?disable');");
    fclose($f);
    $search = "'$'data";
    $replace = (string)"$data";
  ...
  if (isset($_POST['save'])) {
    ...
    $search = array('1' => "$session!$iphost", "$session@|@$userhost", "$session#|#$passwdhost", "$session%$hotspotname", "$session^$dnsname", "$session&$currency", "$session*$areload", "$session($iface", "$session)$maxtx", "$session=$maxrx", "'$session'", "$session@!@$livereport", "$session<$token", "$session?$cid", "$session|?$incomerecap");
    $replace = array('1' => "$sesname!$siphost", "$sesname@|@$suserhost", "$sesname#|#$spasswdhost", "$sesname%$shotspotname", "$sesname^$sdnsname", "$sesname&$scurrency", "$sesname*$sreload", "$sesname($siface", "$sesname)$smaxtx", "$sesname=$smaxrx", "'$sesname'", "$sesname@!@$slivereport", "$sesname<$stoken", "$sesname?$scid", "$sesname|?$sincomerecap");
...
<div class="col-6">
	<div class="card">
        <div class="card-header">
            <h3 class="card-title">Miknam Data</h3>
        </div>
    <div class="card-body">    
	<table class="table table-sm">
	<tr>
