...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if (isset($_POST['name'])) {
    ...
    $onlogin1 = ':put (",rem,' . $price . ',' . $validity . ',' . $graceperiod . ',,' . $getlock . ','. $usetlgrm .',"); {:local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $validity . ');[/system scheduler add disabled=no interval=$uptime name=$user on-event="[/ip hotspot active remove [find where user=$user]];[/ip hotspot user set limit-uptime=1s [find where name=$user]];[/sys sch re [find where name=$user]];[/sys script run [find where name=$user]];[/sys script re [find where name=$user]]" start-date=$date start-time=$time];[/system script add name=$user source=":local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $graceperiod . ');[/system scheduler add disabled=no interval=\$uptime name=$user on-event= \"[/ip hotspot user remove [find where name=$user]];[/ip hotspot active remove [find where user=$user]];[/sys sch re [find where name=$user]]\"]"]';
    $onlogin2 = ':put (",ntf,' . $price . ',' . $validity . ',,,' . $getlock . ','. $usetlgrm .',"); {:local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $validity . ');[/system scheduler add disabled=no interval=$uptime name=$user on-event= "[/ip hotspot user set limit-uptime=1s [find where name=$user]];[/ip hotspot active remove [find where user=$user]];[/sys sch re [find where name=$user]]" start-date=$date start-time=$time]';
    $onlogin3 = ':put (",remc,' . $price . ',' . $validity . ',' . $graceperiod . ',,' . $getlock . ','. $usetlgrm .',"); {:local price ("' . $price . '");:local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $validity . ');[/system scheduler add disabled=no interval=$uptime name=$user on-event="[/ip hotspot active remove [find where user=$user]];[/ip hotspot user set limit-uptime=1s [find where name=$user]];[/sys sch re [find where name=$user]];[/sys script run [find where name=$user]];[/sys script re [find where name=$user]]" start-date=$date start-time=$time];[/system script add name=$user source=":local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $graceperiod . ');[/system scheduler add disabled=no interval=\$uptime name=$user comment=$date-$time on-event= \"[/ip hotspot user remove [find where name=$user]];[/ip hotspot active remove [find where user=$user]];[/sys sch re [find where name=$user]]\"]"];:local bln [:pick $date 0 3]; :local thn [:pick $date 7 11];[:local mac $"mac-address"; :local profile [:put  [/ip hotspot user get $user profile]]; :local comment [:put [/ip hotspot user get $user comment]]; /system script add name="$date-|-$time-|-$user-|-$price-|-$address-|-$mac-|-' . $validity . '-|-$profile-|-$comment" owner="$bln$thn" source=$date comment=miknam]';
    $onlogin4 = ':put (",ntfc,' . $price . ',' . $validity . ',,,' . $getlock . ','. $usetlgrm .',"); {:local price ("' . $price . '");:local date [/system clock get date ];:local time [/system clock get time ];:local uptime (' . $validity . ');[/system scheduler add disabled=no interval=$uptime name=$user on-event= "[/ip hotspot user set limit-uptime=1s [find where name=$user]];[/ip hotspot active remove [find where user=$user]];[/sys sch re [find where name=$user]]" start-date=$date start-time=$time];:local bln [:pick $date 0 3]; :local thn [:pick $date 7 11];[:local mac $"mac-address"; :local profile [:put  [/ip hotspot user get $user profile]]; :local comment [:put [/ip hotspot user get $user comment]]; /system script add name="$date-|-$time-|-$user-|-$price-|-$address-|-$mac-|-' . $validity . '-|-$profile-|-$comment" owner="$bln$thn" source=$date comment=miknam]';

      if($token == "" && $cid == "" && $API->comm("/system/script/print", array("?name" => "ReportPendapatan"))){
          if ($expmode == "rem") {
          ...
          if ($expmode == "rem") {
            ...
            $onlogin = ':put (",,' . $price . ',,,noexp,' . $getlock . ',No,")' . $lock;
          } else {
            $onlogin = "";
          }
        }else{
          if($usetlgrm == "Yes"){
            if ($expmode == "rem") {
              $onlogin = $onlogin1 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
            } elseif ($expmode == "ntf") {
              $onlogin = $onlogin2 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
            } elseif ($expmode == "remc") {
              $onlogin = $onlogin3 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
            } elseif ($expmode == "ntfc") {
              $onlogin = $onlogin4 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
            } elseif ($expmode == "0" && $price != "") {
              $onlogin = ':put (",,' . $price . ',,,noexp,' . $getlock . ',No,")' . $lock;
            } else {
              $onlogin = "";
            ...
            if ($expmode == "rem") {
              $onlogin = $onlogin1 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
            } elseif ($expmode == "ntf") {
              $onlogin = $onlogin2 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
            } elseif ($expmode == "remc") {
              $onlogin = $onlogin3 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
            } elseif ($expmode == "ntfc") {
              $onlogin = $onlogin4 . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
            } elseif ($expmode == "0" && $price != "") {
              $onlogin = ':put (",,' . $price . ',,,noexp,' . $getlock . ',No,")' . $lock;
            } else {
              $onlogin = "";
